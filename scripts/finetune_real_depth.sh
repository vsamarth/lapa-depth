...
export llama_tokenizer_path="${llama_tokenizer_path:-$absolute_path/lapa_checkpoints/tokenizer.model}"
export output_dir="${output_dir:-$absolute_path/outputs}"

export project_id="${project_id:-lapa}"
export experiment_note="${experiment_note:-lapa_finetune_depth}"

export dataset_path="${dataset_path:-$absolute_path/data/real_finetune.jsonl}"
export experiment_id="${experiment_id:-finetune_real_depth}"
...
python3 -u -m latent_pretraining.train \
    ...
    --update_llama_config="dict(action_vocab_size=256,depth_vocab_size=8448,theta=50000000,max_sequence_length=640,use_flash_attention=True,scan_attention=True,scan_query_chunk_size=512,scan_key_chunk_size=1024,remat_attention='nothing_saveable',scan_mlp=True,scan_mlp_chunk_size=8192,remat_mlp='nothing_saveable',remat_block='nothing_saveable',scan_layers=True)" \
    --tokenizer.vocab_file="$llama_tokenizer_path" \
    --optimizer.type='adamw' \
    --llama.action_vocab_size=256 \
    --llama.depth_vocab_size=8448 \
    ...
    --train_dataset.vision_depth_action_processor.vqgan_checkpoint_path="$absolute_path/lapa_checkpoints/vqgan" \
    --train_dataset.vision_depth_action_processor.image_absolute_path="$absolute_path/data/" \
    --train_dataset.vision_depth_action_processor.max_n_frames=1 \
    --train_dataset.json_vision_depth_action_dataset.mode="pad" \
    --train_dataset.json_vision_depth_action_dataset.path="$dataset_path" \
    --train_dataset.json_vision_depth_action_dataset.seq_length=640 \
    --train_dataset.json_vision_depth_action_dataset.batch_size=64 \
