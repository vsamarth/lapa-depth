...
class VideoLLaMAConfig(LLaMAConfig):
    ...
    def __init__(self, vision_vocab_size=8448, depth_vocab_size=8448, tie_vision_embeddings=False, action_vocab_size=256, sample_mode='all', **kwargs):
        super().__init__(**kwargs)
        self.vision_vocab_size = vision_vocab_size # 8192 + 256
        self.depth_vocab_size = depth_vocab_size
        ...
        if scan_layers:
            if scan_axis == 0:
                return (
                    ...
                    ("transformer/ate/embedding", PS("tp", ("fsdp", "sp"))),
...
class FlaxVideoLLaMAPreTrainedModel(FlaxPreTrainedModel):
    ...
    def init_cache(self, batch_size, max_length):
        ...
        vision_masks = jnp.zeros((batch_size, max_length), dtype=bool)
        depth_masks = jnp.zeros((batch_size, max_length), dtype=bool)
        action_masks = jnp.zeros((batch_size, max_length), dtype=bool)

        init_variables = self.module.init(
            jax.random.PRNGKey(0), input_ids, vision_masks, depth_masks, action_masks, attention_mask, segment_ids, position_ids, return_dict=False, init_cache=True
        )
    ...
    def init_weights(self, rng, input_shape, params=None):
        ...
        attention_mask = jnp.ones_like(input_ids)
        vision_masks = jnp.zeros(input_ids.shape, dtype=bool)
        depth_masks = jnp.zeros(input_ids.shape, dtype=bool)

        action_masks = jnp.zeros(input_ids.shape, dtype=bool)
        segment_ids = jnp.zeros_like(input_ids)
        position_ids = jnp.broadcast_to(jnp.arange(jnp.atleast_2d(input_ids).shape[-1]), input_shape)
        params_rng, dropout_rng = jax.random.split(rng)
        rngs = {"params": params_rng, "dropout": dropout_rng}

        random_params = self.module.init(rngs, input_ids, vision_masks, depth_masks, action_masks, attention_mask, segment_ids, position_ids, return_dict=False)["params"]
    ...
    def __call__(
        ...
        outputs = self.module.apply(
            ...
            jnp.array(depth_masks, dtype="f4"),
        ...
        # print("outputs logits, llm?", outputs[0])
...
class FlaxVideoLLaMAModule(nn.Module):
    ...
    def __call__(
        ...
        if input_ids.shape[1] == 1:
            ...
            elif self.config.sample_mode == 'depth':
                input_embeds = self.dte(input_ids)
            elif self.config.sample_mode == 'action':
                input_embeds = self.ate(input_ids)
            elif self.config.sample_mode == 'all':
                raise NotImplementedError
            else:
                raise ValueError(f"Invalid sample_mode: {self.config.sample_mode}")
        else:
            input_text_embeds = self.wte(input_ids)
            input_vision_embeds = self.vte(jnp.where(vision_masks, input_ids, 0))
            input_depth_embeds = self.dte(jnp.where(depth_masks, input_ids, 0))
            input_action_embeds = self.ate(jnp.where(action_masks, input_ids, 0))

            vision_masks = vision_masks[..., None].astype("f4")
            depth_masks = depth_masks[..., None].astype("f4")
            action_masks = action_masks[..., None].astype("f4")

            text_masks = 1.0 - jnp.clip(vision_masks + depth_masks + action_masks, 0.0, 1.0)

            input_embeds = (
                input_text_embeds * text_masks
                + input_vision_embeds * vision_masks
                + input_depth_embeds * depth_masks
                + input_action_embeds * action_masks
            )
...
class FlaxActionLaMAForCausalLMModule(nn.Module):
    config: VideoLLaMAConfig
    dtype: jnp.dtype = jnp.float32
    param_dtype: jnp.dtype=jnp.float32
    precision: Optional[Union[jax.lax.Precision, str]]=None
    ...
    def __call__(
        ...
        if self.config.sample_mode == 'all':
            if not return_dict:
                return (vision_logits, lm_logits, action_logits) + outputs[1:]

            return FlaxCausalLMOutput(logits=(vision_logits, lm_logits, action_logits), hidden_states=outputs.hidden_states, attentions=outputs.attentions)
        elif self.config.sample_mode == 'vision':
            if not return_dict:
            ...
            return FlaxCausalLMOutput(logits=action_logits, hidden_states=outputs.hidden_states, attentions=outputs.attentions)
        else:
            raise ValueError(f"Invalid sample_mode: {self.config.sample_mode}")
...
class FlaxVideoLLaMAForCausalLM(FlaxVideoLLaMAPreTrainedModel):
    module_class = FlaxActionLaMAForCausalLMModule
    ...
    def update_inputs_for_generation(self, model_outputs, model_kwargs):
        return {
            ...
            "depth_masks": model_kwargs["depth_masks"],
    ...
    def _sample_vision(
        ...
        def sample_search_body_fn(state):
            ...
            # eos_token does not mean eos for delta
