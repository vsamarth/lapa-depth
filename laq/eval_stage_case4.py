...
def load_model(checkpoint_path, adapter_path, device):
    ...
    # 1) load full LAQ / VAE checkpoint
    ...
    # 2) load Case-4 adapter checkpoint
    adapter_ckpt = torch.load(adapter_path, map_location=device)
    model.rgb_token_embed.load_state_dict(adapter_ckpt["rgb_token_embed"])
    model.geometry_adapter.load_state_dict(adapter_ckpt["geometry_adapter"])

    print(f"Loaded adapter from: {adapter_path}")
    if "meta" in adapter_ckpt:
        print("Adapter meta:", adapter_ckpt["meta"])
...
def main():
    ...
    checkpoint_path = "/home/do/Workspace/philo/External/LAPA/laq/results_case4/vae.9500.pt"
    adapter_path = "/home/do/Workspace/philo/External/LAPA/laq/results_case4/adapter.9500.pt"

    offsets = 30
    batch_size = 1
    num_workers = 0
    modality = "both"   # Case 4: depth latent + geometry-adapted z_rgb -> predict future depth

    output_dir = Path("eval_case4_outputs")
    ...
    # ======== Model ========
    model = load_model(checkpoint_path, adapter_path, device)
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(dl):
            ...
            if modality == "rgb":
                ...
                # Case 4:
                # current depth + z_rgb_index -> token embed + geometry adapter
                # -> fuse with z_depth -> predict future depth
                video = depth
                decoder_video = depth
            ...
            if batch_idx % save_every == 0 and vis_saved < max_vis_batches:
                save_path = output_dir / f"{ckpt_name}_{adapter_name}_batch_{batch_idx:05d}.png"
                save_vis(
    ...
    metrics_path = output_dir / f"{ckpt_name}_{adapter_name}_metrics.json"
