...
class LatentActionQuantization(nn.Module):
    ...
    def forward(
        ...
        *_, h, w, _ = shape
        ...
        # print("depth indices, tokens: ", indices.shape, tokens.shape)
        
        z_depth = tokens
        
        B = tokens.shape[0]   # 64

        # fake_index_single = torch.tensor([6, 4, 1, 4], device=tokens.device)
        # fake_index = fake_index_single.unsqueeze(0).repeat(B, 1)   # [64, 4]
        if z_rgb is not None: 
            z_rgb_latent = self.vq.indices_to_tokens(z_rgb, reshape_to_grid=False)

        # print("fake_index: ", fake_index.shape)
        # print("z_rgb: ", z_rgb.shape)
        # print(" z_rgb_latent, tokens: ", z_rgb_latent.shape, tokens.shape)
        ...
        if z_rgb is not None:
            z_fused = 0.5 * z_depth + 0.5 * z_rgb_latent
