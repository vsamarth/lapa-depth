...
def main():
    # ======== Config ========
    rgb_path = "/media/do/data1/philo/lapa/something-something-v2/frames_val"
    depth_path = "/media/do/data1/philo/lapa/something-something-v2/depth_val"
    z_rgb_path = "/media/do/data1/philo/lapa/something-something-v2/pred_z_rgb_step1_val"

    checkpoint_path = "results-depth only/vae.10000.pt"
    offsets = 12
    batch_size = 1
    num_workers = 0
    modality = "depth"

    output_dir = Path("eval_stage1_outputs")
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(dl):
            ...
            if batch_idx % save_every == 0 and vis_saved < max_vis_batches:
                save_path = output_dir / f"{ckpt_name}_batch_{batch_idx:05d}.png"
                save_vis(
                    ...
                    nrow=3,
                    max_items=1,   # batch_size=1 nên để 1 cho gọn
                )
                vis_saved += 1
    ...
    metrics_path = output_dir / f"{ckpt_name}_metrics.json"
